...
#define ZERO_CROSSINGS_HYSTERESIS_KF             0.3f
#define ZERO_CROSSINGS_VELOCITY_THRESHOLD_KF     0.15f
...
public:
    ...
    void correct(float accel, float delta_t) {
        if (zero_crossing_detected) {
            ...
            Vector2f z;
            const float freq_guess = M_PI / std::max(zero_crossing_last_interval, 0.2f);  //  rad/s
            float new_y = x(1);    
