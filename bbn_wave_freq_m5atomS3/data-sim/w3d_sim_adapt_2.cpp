...
const float FAIL_ERR_LIMIT_PERCENT_X_HIGH = 67.0f;
const float FAIL_ERR_LIMIT_PERCENT_Y_HIGH = 67.0f;
const float FAIL_ERR_LIMIT_PERCENT_Z_HIGH = 15.0f;

const float FAIL_ERR_LIMIT_PERCENT_X_LOW  = 67.0f;
    ...
    reader.for_each_record([&](const Wave_Data_Sample &rec) {
        ...
        if (std::isfinite(f_hz) && rec.time >= ONLINE_TUNE_WARMUP_SEC) {
            ...
            if (tuner.isReady()) {
                ...
                RS_target = std::clamp(C_adj * tuner.getR_S_est(), MIN_R_S, MAX_R_S);
            }
        }
