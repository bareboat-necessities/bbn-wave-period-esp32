...
const float FAIL_ERR_LIMIT_PERCENT_3D_PMSTOKES = 65.0f;
const float FAIL_ACC_Z_BIAS_PERCENT = 30.0f;
const float FAIL_ERR_LIMIT_BIAS_3D_PERCENT = 320.0f;
...
static void process_wave_file_for_tracker(const std::string& filename, float dt, bool with_mag)
{
    ...
    FusionAdapter3 adapter(with_mag, sigma_a_init, sigma_g, sigma_m, mag_world_a);
