                                   ...
                                   unsigned run_seed)
{
    ...
    std::string trackerName =
        ...
        (tracker == TrackerType::KALMANF)   ? "kalmanf" : "zerocross";
    ...
    char noise_bias[64];
    std::snprintf(noise_bias, sizeof(noise_bias), "_N%.3f_B%.3f", NOISE_STDDEV, BIAS_MEAN);
    std::string outFile = "tuning_" + trackerName + "_" + waveName + tail + noise_bias + ".csv";
...
int main() {
    ...
    std::cout << "All MiniTuningEstimator runs complete.\n";
    return 0;
}
