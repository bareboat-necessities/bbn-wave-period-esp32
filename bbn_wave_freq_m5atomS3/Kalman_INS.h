...
/*
  ...
  q-mekf (merged extension, INS variant)

  Enhancements / INS extension (Matérn-3/2 latent acceleration):
  Copyright (c) 2025

  This file provides a quaternion MEKF with extended linear navigation states:
     v (3)   : velocity in world frame
     p (3)   : displacement/position in world frame
     S (3)   : integral of displacement (∫ p dt) — pseudo-measured to 0 for drift control
     a_w (3) : latent world-frame acceleration (Matérn-3/2; a_w ≡ a)
     j_w (3) : latent world-frame acceleration rate (j_w ≡ \dot a)

  Differences vs the OU version:
    • The latent acceleration model is now second-order Gauss–Markov (Matérn-3/2):
        ȧ = j,   j̇ = -(2/τ) j - (1/τ²) a + w,     E[w wᵀ] = (4/τ³) Σ_stat δ(t-s)
      which yields stationary Var[a] = Σ_stat.
    • Exact discretization of the (v,p,S,a,j) block via Van Loan.
*/
