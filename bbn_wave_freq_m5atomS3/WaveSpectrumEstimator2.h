...
private:
  ...
  void computeSpectrum_() {
    ...
    const double a_lin = (std::fabs(denom) > 1e-18) ? ((double(N) * sumnx - sumn * sumx) / denom) : 0.0;
    ...
    const double f_blk = 1.0 / std::max(1e-6, 6.0 * Tblk);
    ...
    for (int i = 0; i < Nfreq; ++i) {
      ...
      // Guard term grows at low f to avoid exploding deconvolution
      const double eps_H = 0.02 + 0.5 * (0.05 / (f + 1e-6));
      const double S_aa_true = S_aa_meas / (H2 + eps_H);

      // Regularized displacement conversion: S_eta = S_aa / (w^2 + wr^2)^2
      const double f_knee = std::max(cfg_.reg_f0_hz, f_blk);
      const double wr = 2.0 * kPi * std::max(f_knee, 0.6 * f);
      ...
      // Gentle low-f guard (still allow energy, just suppress pathological floor)
      ...
      if (cfg_.psd_ema_enable) {
        ...
        else                psd_ema_[i] = (1.0 - cfg_.psd_ema_alpha) * psd_ema_[i] + cfg_.psd_ema_alpha * S_eta;
