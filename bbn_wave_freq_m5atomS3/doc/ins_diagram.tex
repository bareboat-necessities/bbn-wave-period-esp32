...
\begin{tikzpicture}[node distance=2cm,
    ...
    \draw[line] (ahrs.west) -| (roll_pitch.north) node[pos=0.75, above, sloped] {$a_x, a_y$};
    ...
    \draw[line] (freq.east) -| (sea_state.north) node[midway, above] {$f_{est}$};
    ...
    % Multiple IMU → AHRS inputs: no crossing
    \draw[line] (imu.south west) |- (ahrs.north west) node[pos=0.75, above] {gyro};
    \draw[line] (imu.south) -- (ahrs.north) node[midway, right] {magn};
    \draw[line] (imu.south east) |- (ahrs.north east) node[pos=0.75, above] {accel};
