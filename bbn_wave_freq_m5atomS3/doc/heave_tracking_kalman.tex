...
\begin{itemize}
  ...
  \item $F_k$: State transition matrix
  \item $B_k$: Control input matrix
  \item $\bm{u}_k$: Control input vector at time $k$
  \item $P_{k|k-1}$: Predicted state covariance
  \item $P_{k-1|k-1}$: Posterior state covariance at time $k-1$
  \item $Q_k$: Process noise covariance matrix
\end{itemize}
...
\begin{itemize}
  ...
  \item $H_k$: Observation matrix
  \item $\bm{y}_k$: Innovation (residual), difference between measurement and prediction
  \item $S_k$: Innovation covariance
  \item $R_k$: Measurement noise covariance
  \item $K_k$: Kalman gain
  \item $\bm{x}_{k|k}$: Updated (posterior) state estimate at time $k$
  \item $P_{k|k}$: Updated (posterior) estimate covariance using the Joseph form
  \item $I$: Identity matrix of appropriate size
\end{itemize}
