...
\subsection*{Kalman Filter Equations (Linear, Discrete-Time)}
...
\begin{equation}
    z^{\text{meas}}_k = \mathbf{H}_k \, \mathbf{x}_k + \mathbf{\nu}_k
\end{equation}
\begin{equation}
    \mathbf{\nu}_k \sim \mathcal{N}(\mathbf{0}, R_k)
\end{equation}
...
\begin{itemize}
    ...
    \item \( z^{\text{meas}}_k \in \mathbb{R}^p \): measurement vector
    \item \( F_k \in \mathbb{R}^{n \times n} \): state transition matrix
    \item \( B_k \in \mathbb{R}^{n \times m} \): control input matrix
    \item \( H_k \in \mathbb{R}^{p \times n} \): observation matrix
    \item \( Q_k \in \mathbb{R}^{n \times n} \): process noise covariance
    \item \( R_k \in \mathbb{R}^{p \times p} \): measurement noise covariance
\end{itemize}
...
\begin{align}
    \mathbf{y}_k &= z^{\text{meas}}_k - \mathbf{H}_k \, \hat{\mathbf{x}}_{k|k-1} \\
...
\paragraph{Notes:}
Equation (8) is the \textbf{Joseph stabilized form} of the covariance update, which maintains positive semi-definiteness and improves numerical stability.
