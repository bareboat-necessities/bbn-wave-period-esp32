...
\begin{equation}
    \bm{z}^{\text{meas}}_k = \bm{H}_k \, \bm{x}_k + \boldsymbol{\nu}_k
\end{equation}
\begin{equation}
    \boldsymbol{\nu}_k \sim \mathcal{N}(\bm{0}, \bm{R}_k)
\end{equation}

\paragraph{Variables:}
\begin{itemize}
    \item \( \bm{x}_k \in \mathbb{R}^n \): state vector
    \item \( \bm{u}_k \in \mathbb{R}^m \): control input
    \item \( \bm{z}^{\text{meas}}_k \in \mathbb{R}^p \): measurement vector
    \item \( \bm{F}_k \in \mathbb{R}^{n \times n} \): state transition matrix
    \item \( \bm{B}_k \in \mathbb{R}^{n \times m} \): control input matrix
    \item \( \bm{H}_k \in \mathbb{R}^{p \times n} \): observation matrix
    \item \( \bm{Q}_k \in \mathbb{R}^{n \times n} \): process noise covariance
    \item \( \bm{R}_k \in \mathbb{R}^{p \times p} \): measurement noise covariance
\end{itemize}
...
\begin{align}
    \hat{\bm{x}}_{k|k-1} &= \bm{F}_k \, \hat{\bm{x}}_{k-1|k-1} + \bm{B}_k \, \bm{u}_k \\
...
\begin{itemize}
  ...
  \item $\bm{F}_k$: State transition matrix
  \item $\bm{B}_k$: Control input matrix
  \item $\bm{u}_k$: Control input vector at time $k$
  \item $\bm{P}_{k|k-1}$: Predicted state covariance
  \item $\bm{P}_{k-1|k-1}$: Posterior state covariance at time $k-1$
  \item $\bm{Q}_k$: Process noise covariance matrix
\end{itemize}

\subsubsection*{Update Step (Measurement Update)}
\begin{align}
    \bm{y}_k &= \bm{z}^{\text{meas}}_k - \bm{H}_k \, \hat{\bm{x}}_{k|k-1} \\
    ...
    \bm{P}_{k|k} &= (\bm{I} - \bm{K}_k \bm{H}_k) \, \bm{P}_{k|k-1} \, (\bm{I} - \bm{K}_k \bm{H}_k)^\top + \bm{K}_k \bm{R}_k \bm{K}_k^\top
\end{align}

\textbf{Where:}
\begin{itemize}
  \item $\bm{z}^{\text{meas}}_k$: Measurement vector at time $k$
  \item $\bm{H}_k$: Observation matrix
  \item $\bm{y}_k$: Innovation (residual), difference between measurement and prediction
  \item $\bm{S}_k$: Innovation covariance
  \item $\bm{R}_k$: Measurement noise covariance
  \item $\bm{K}_k$: Kalman gain
  \item $\bm{x}_{k|k}$: Updated (posterior) state estimate at time $k$
  \item $\bm{P}_{k|k}$: Updated (posterior) estimate covariance using the Joseph form
  \item $\bm{I}$: Identity matrix of appropriate size
\end{itemize}

\paragraph{Notes:}
Equation for \( {\bm{P}}_{k|k} \) is the \textbf{Joseph stabilized form} of the covariance update, which maintains positive semi-definiteness and improves numerical stability.
...
The continuous-time vertical acceleration \( a = a(t) \) is sampled at time steps \( k \) with period \( T \). The state vector \( \bm{x}_k \in \mathbb{R}^4 \) is defined as:
\begin{equation}
...
The matrices \( \bm{F}_k \) (state transition) and \( \bm{B}_k \) (input matrix) are time step dependent:
\begin{equation}
\bm{F}_k =
\begin{bmatrix}
...
\begin{equation}
\bm{H} = \begin{bmatrix} 1 & 0 & 0 & 0 \end{bmatrix}
\end{equation}
...
\item\textbf{Predict}
\begin{itemize}
   \item State Prediction (\( \bm{\hat{x}}_{k|k-1} \))  
   \item Covariance Prediction (\( \bm{P}_{k|k-1} \))  
\end{itemize}
\item\textbf{Update}
\begin{itemize}
   \item Innovation Calculation (\( \bm{y}_k \))  
   \item Innovation Covariance Calculation (\( \bm{S}_k \))  
   \item Kalman Gain Calculation (\( \bm{K}_k \))  
   \item State Update (\( \bm{\hat{x}}_{k|k} \))  
   \item Covariance Update (\( \bm{P}_{k|k} \))
\end{itemize}
\end{enumerate}

This cycle repeats iteratively as new measurements arrive. 

\subsection*{Numerical Stabilization}
\begin{itemize}
\item Symmetry: $\bm{P} \leftarrow \frac{1}{2}(\bm{P}+\bm{P}^\top)$
\item Positive definite enforcement: While $\bm{P} \not\succ 0$, add $\epsilon_1\bm{I}$, where $\epsilon_1$ positive and close to zero (like $10^{-12}$)
\item Numerical safety: $\bm{S}^{-1}$ clamped at $\epsilon_2$ ($10^{-12}$)
\end{itemize}
...
where $\Delta t$ is time since the last crossing.
...
Estimation of frequency is given by equation~\eqref{eq:freqEst} using time since the last zero crossing.
...
\subsubsection*{Kalman Update and Mahalanobis Gating}
