...
&\bm{K}_k = \bm{P}_k^- H_k^\top
  \bigl(H_k\bm{P}_k^-H_k^\top + \bm{R}\bigr)^{-1},\\
&\bm{x}_k = \bm{x}_k^- + \bm{K}_k\bigl(\vec{z}_k - H_k\bm{x}_k^-\bigr),\\
&\bm{P}_k
= (\bm{I}-\bm{K}_kH_k)\,\bm{P}_k^-\,(\bm{I}-\bm{K}_kH_k)^\top
...
The filter’s state \(\bm{x}_k\) absorbs that unknown offset into its estimate of \(\vec{A}\), and by minimizing residuals over time it converges to the correct direction.  In effect, the Kalman update \emph{self‐corrects} for unknown phase by adjusting both components of \(\bm{x}_k\) so that \(H_k\bm{x}_k\approx\vec{a}_k\) in the least‐squares sense, yielding the true ratio \(A_y/A_x\).

\section{Additional Numerical Safeguards}
\begin{itemize}
  \item \textbf{Mahalanobis gating:} Reject updates if the innovation \(\vec{z}_k - H_k\bm{x}_k^-\) exceeds a threshold in Mahalanobis distance.
  \item \textbf{LDLT decomposition:} Use LDLT factorization for inverting \(H_k\bm{P}_k^-H_k^\top + \bm{R}\), enhancing robustness against ill-conditioning.
...
\begin{itemize}
  ...
  \item Filtered acceleration: \(H_k\,\bm{x}_k\)
