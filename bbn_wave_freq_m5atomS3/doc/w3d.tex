...
% Two-column helpers
\usepackage{multicol}  % not strictly needed now, but harmless if you want local multi-cols later
\setlength{\columnsep}{18pt} % spacing between columns
\newcommand{\fitcol}[1]{\adjustbox{max width=\columnwidth}{$\displaystyle #1$}}
\usepackage{cuted}     % optional: \begin{strip} ... \end{strip} for full-width inserts
...
\pdfstringdefDisableCommands{%
  \def\SI#1#2{#1 #2}%
  \def\bm#1{#1}%  % drop bold in bookmarks
  \def\Skew#1{[#1]_\text{x}}% safer text-y substitute
  \def\Id{I}%
  \def\pct#1{#1\%}%
}

% Header bold SMALL; body SMALL upright; compact spacing
\newtheoremstyle{bodysmall}% name
  {3pt}{3pt}% space above/below
  {\normalfont\small}% body font
  {0pt}% indent
  {\bfseries\small}% head font
...
\pdfstringdefDisableCommands{%
  ...
  \def\Phi{Phi}%
  \def\omega{omega}%
  \def\_{}% drop subscripts in bookmarks
  \def\cdot{}%
  \def\;{ }%
}
...
% Make title + abstract + ToC span both columns
\twocolumn[
\maketitle
\begin{onecolumngrid}
\begin{abstract}
