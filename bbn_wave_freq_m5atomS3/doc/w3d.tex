...
\DeclareRobustCommand{\vct}[1]{\bm{#1}}     % vectors (Latin & Greek): bold-italic
\DeclareRobustCommand{\mat}[1]{\mathbf{#1}} % LATIN matrices: upright bold
\DeclareRobustCommand{\matg}[1]{\bm{#1}}    % GREEK matrices: bold-italic
\DeclareRobustCommand{\skew}[1]{\left[\vct{#1}\right]_\times}
...
an Ornstein–Uhlenbeck (OU) process, giving stationary variance and realistic temporal correlation. Gyroscope bias is modeled a random walk; 
...
using the tracked carrier frequency. We also outline practical tuning and an adaptive law that scales the pseudo-measurement 
variance with the OU parameters, enabling stable behavior across sea states. The approach targets embedded, low-cost IMU deployments, 
supporting dynamic positioning, weather and wave buoys, maritime safety, and oceanographic studies that require reliable heave 
and wave direction in real time.
\end{abstract}
...
%\newpage
%\section*{Appendix}
%Symbolical derivations for discretization coefficients matrices.
%\input{w3d-lyap.tex-part}

%\input{w3d-adapt-spec.tex-part}
