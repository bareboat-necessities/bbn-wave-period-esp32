...
\medskip
All other partial derivatives are zero.  In the C++ code \linebreak
...
\begin{itemize}
  ...
  \item Computes DCT‐I transforms in \linebreak \verb|FentonFFT<N>::compute_inverse_cosine_transform()|.
  \item Handles hyperbolic functions with numerically stable helpers \verb|sinh_by_cosh| and \verb|cosh_by_cosh|.
  \item Solves the dispersion relation for $\lambda$ via Newton iteration in \linebreak\verb|compute_wavelength()|.
...
In addition to wave kinematics, the C++ implementation provides a \linebreak
\texttt{WaveSurfaceTracker<N,Real>} class that simulates the motion of a small floating body constrained to the wave surface.  The model assumes:
...
\linebreak The class uses a classical 4th‐order Runge–Kutta scheme (RK4) with time step \(\Delta t\), updating both \(x\) and \(v_x\) simultaneously:
\[
\begin{aligned}
  k_{1}^{(x)} &= v_x, 
  &\quad
