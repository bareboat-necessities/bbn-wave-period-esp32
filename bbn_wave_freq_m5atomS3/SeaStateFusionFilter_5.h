...
/*
  ...
  Combines multiple real-time estimators into a cohesive ocean-state tracker:
    ...
    • Online auto-tuning of motion-strength and pseudo-measurement parameters
      through SeaStateAutoTuner, using dominant-frequency and vertical-variance
      estimates to retune accel noise plus p/v drift suppression.

  Where
  – τ (tau):  wrapper-only dominant-period heuristic ≈ ½ · T
  – σₐ:       applied accel-noise / motion-strength proxy, EWMA-tracked online
...
namespace seastate_fusion5_detail {
...
constexpr float MIN_TAU_S     = 0.02f;  // sec, wrapper-only heuristic bound
constexpr float MAX_TAU_S     = 3.0f;   // sec, wrapper-only heuristic bound
...
struct TuneState {
    float tau_applied      = 1.1f;   // s, wrapper-only sea-state heuristic
...
public:
    ...
    using TrackingPolicy = seastate_fusion5_detail::TrackerPolicy<trackerT>;
    ...
    explicit SeaStateFusionFilter_5(bool with_mag = true)
        : with_mag_(with_mag),
          time_(0.0),
    ...
    {
        mekf_ = std::make_unique<Kalman3D_Wave_5<float>>(sigma_a, sigma_g, sigma_m);
    ...
    {
        mekf_ = std::make_unique<Kalman3D_Wave_5<float>>(sigma_a, sigma_g, sigma_m, Pq0, Pb0, b0, R_p0_var_init, R_v0_var_init, gravity_magnitude);
        enterCold_();      // applies freeze + warmup Racc + disables linear block
        apply_filter_tune_();
        mekf_->set_exact_att_bias_Qd(true);
    } 
                    ...
                    float tempC = 35.0f)
    {
        ...
        // MEKF updates first (attitude + world-frame accel-bias state b_aw)
    ...
    // P-factor scales horizontal vs vertical command-noise std applied to the b_aw model.
    ...
    // Enable/disable use of the extended linear block [v,p,S,b_aw] in Kalman3D_Wave_5.
    //
    // When flag == false:
    //   • Kalman3D_Wave_5::set_linear_block_enabled(false) is enforced,
    //   • startup stages still progress (Cold → TunerWarm → Live),
    //   • tuner still runs and exposes dominant-period heuristics plus sigma/R_p0/R_v0 targets & metrics,
    //   • but v/p/b_aw propagation and pseudo-measurements are never used.
    //
    ...
    // For SeaStateFusionFilter_5 to restore Racc automatically
    ...
    // Wrapper-only sea-state heuristic; not a Kalman3D_Wave_5 core state parameter.
    inline float getTauApplied()        const noexcept { return tune_.tau_applied; }
    // Accel-noise sigma currently applied to Kalman3D_Wave_5.
    inline float getSigmaApplied()      const noexcept { return mekf_ ? mekf_->get_Racc_std().z() : tune_.sigma_applied; }
    ...
    // Empirical sea-state heuristic derived from wrapper tau/sigma estimates.
...
private:
    ...
    struct StillnessAdapter {
        ...
        float target_freq_hz  = seastate_fusion5_detail::MIN_FREQ_HZ; // relaxed target (could also be FREQ_GUESS)
    ...
    void apply_filter_tune_() {
        if (!mekf_) return;
        // Wrapper smoothing is the only active parameter-smoothing layer here;
        // Kalman3D_Wave_5 param-RW stays disabled. Sigma is applied as command-space
        // accel noise, which also sets the b_aw random-walk scaling inside the core.
        ...
        mekf_->set_Racc_std(Eigen::Vector3f(sH, sH, sZ));
    }
    ...
    void update_tuner(float dt, float a_vert_inertial, float freq_hz_for_tuner) {
        ...
        // tau_target_ remains a wrapper-only dominant-period heuristic used to scale
        // pseudo-measurement suppression; it is not pushed into Kalman3D_Wave_5.
    ...
    void adapt_mekf(float dt, float tau_heuristic_t, float sigma_t, float R_p0_t, float R_v0_t) {
        const float alpha = 1.0f - std::exp(-dt / adapt_tau_sec_);
    
        // R_p0 / R_v0 smoothing still depends on the wrapper period heuristic.
        const float R_p0_sec   = seastate_fusion5_detail::ADAPT_R_p0_MULT * tau_heuristic_t;
        const float R_v0_sec   = seastate_fusion5_detail::ADAPT_R_v0_MULT * tau_heuristic_t;
        const float alpha_R_p0 = 1.0f - std::exp(-dt / R_p0_sec);
        const float alpha_R_v0 = 1.0f - std::exp(-dt / R_v0_sec);
    
        tune_.tau_applied       += alpha      * (tau_heuristic_t - tune_.tau_applied);
    ...
    void resetTrackingState_() {
        ...
        freq_fast_smoother_   = FirstOrderIIRSmoother<float>(seastate_fusion5_detail::FREQ_SMOOTHER_DT, 3.5f);
        freq_slow_smoother_   = FirstOrderIIRSmoother<float>(seastate_fusion5_detail::FREQ_SMOOTHER_DT, 10.0f);
    ...
    float speed_env_mult_ = 1.0f;   // heuristic multiplier for vertical-speed envelope reporting

    // Controls whether the extended linear block [v,p,b_aw] of Kalman3D_Wave_5
    // is ever enabled. When false, the underlying filter runs as a pure
    // attitude/bias QMEKF (linear states frozen, no p/v pseudo-measurements),
    ...
    float min_freq_hz_            = seastate_fusion5_detail::MIN_FREQ_HZ;
    float max_freq_hz_            = seastate_fusion5_detail::MAX_FREQ_HZ;
    float min_tau_s_              = seastate_fusion5_detail::MIN_TAU_S;
    float max_tau_s_              = seastate_fusion5_detail::MAX_TAU_S;
    float max_sigma_a_            = seastate_fusion5_detail::MAX_SIGMA_A;
    float MIN_R_p0_std_           = seastate_fusion5_detail::MIN_R_p0_std;
    float MAX_R_p0_std_           = seastate_fusion5_detail::MAX_R_p0_std;
    float MIN_R_v0_std_           = seastate_fusion5_detail::MIN_R_v0_std;
    float MAX_R_v0_std_           = seastate_fusion5_detail::MAX_R_v0_std;
    float adapt_tau_sec_          = seastate_fusion5_detail::ADAPT_TAU_SEC;
    float adapt_every_secs_       = seastate_fusion5_detail::ADAPT_EVERY_SECS;
    float online_tune_warmup_sec_ = seastate_fusion5_detail::ONLINE_TUNE_WARMUP_SEC;
    float mag_delay_sec_          = seastate_fusion5_detail::MAG_DELAY_SEC;

    // Runtime-configurable anisotropy knobs
    float R_p0_xy_factor_ = 0.23f;  // [0..1] scales XY pseudo-meas vs Z
    float P_factor_       = 1.5f;   // (>0) scales horizontal vs vertical accel-noise tuning

    TrackingPolicy                  tracker_policy_{};
    FirstOrderIIRSmoother<float>    freq_fast_smoother_{seastate_fusion5_detail::FREQ_SMOOTHER_DT, 3.5f};   // ~3.5 s to 90% step
    FirstOrderIIRSmoother<float>    freq_slow_smoother_{seastate_fusion5_detail::FREQ_SMOOTHER_DT, 10.0f};  // ~10 s to 90% step
    SeaStateAutoTuner               tuner_;
    seastate_fusion5_detail::TuneState                       tune_;

    float tau_target_   = NAN; // wrapper-only dominant-period heuristic
    float sigma_target_ = NAN;
    float R_p0_std_target_  = NAN;
    float R_v0_std_target_  = NAN;

    // Runtime-configurable accel noise floor (1σ), m/s²
    float acc_noise_floor_sigma_ = seastate_fusion5_detail::ACC_NOISE_FLOOR_SIGMA_DEFAULT;

    float R_p0_coeff_   = 2.2f;
    float R_v0_coeff_   = 1.6f;
    float tau_coeff_    = 1.5f; // wrapper-only period heuristic coefficient
    float sigma_coeff_  = 0.85f;  // Real noise inflates estimated sigma, so we slightly down-scale the command-noise estimate.

    std::unique_ptr<Kalman3D_Wave_5<float>>  mekf_;
...
public:
    ...
    struct Config {
        ...
        // Init / staging
        float mag_delay_sec          = seastate_fusion5_detail::MAG_DELAY_SEC;
        float online_tune_warmup_sec = seastate_fusion5_detail::ONLINE_TUNE_WARMUP_SEC;
    ...
    void update(float dt, const Eigen::Vector3f& gyro_body_ned, const Eigen::Vector3f& acc_body_ned, float tempC = 35.0f)
    {
        ...
        if (cur_stage != last_impl_startup_stage_) {
            if (cur_stage == SeaStateFusionFilter_5<trackerT>::StartupStage::Cold) {
    ...
    SeaStateFusionFilter_5<trackerT>& raw() { return impl_; }
...
private:
    Config cfg_{};
    SeaStateFusionFilter_5<trackerT> impl_{false};
