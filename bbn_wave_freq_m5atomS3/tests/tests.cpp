...
KalmANF kalmANF;
SchmittTriggerFrequencyDetector freqDetector(0.0005f); // Hysteresis
...
void run_filters(float a, float v, float h, float delta_t) {
  ...
  if (t > warmup_time_sec(true)) {
    ...
    if (useFrequencyTracker == Aranovskiy) {
      ...
      float f_byZeroCross = freqDetector.update(a, 2.0, delta_t); 
      freq = f_byZeroCross;
    }
  ...
  if (freq_adj > FREQ_LOWER && freq_adj < FREQ_UPPER) { /* prevent decimal overflows */
    ...
    printf(",heave_alt,%.4f", isnan(waveAltState.heave) ? 0.0f : waveAltState.heave);
