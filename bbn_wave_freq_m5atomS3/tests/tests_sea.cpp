...
static constexpr int WARMUP_STEPS = 100; // warmup steps before SeaStateRegularity
...
static Wave_Sample sample_fenton(double t, FentonWave<ORD> &fenton) {
    return { fenton.surfaceVerticalAcceleration(static_cast<float>(t)) };
}
...
// Process a single sample with SeaStateRegularity
...
// Warmup trackers before feeding SeaStateRegularity
template<typename SampleFunc>
static void warmup_trackers(SampleFunc sample_func, double &sim_t, TrackerType tracker,
                            float bias, std::default_random_engine &rng,
...
static void run_one_scenario(WaveType waveType, TrackerType tracker,
                             const WaveParameters &wp, unsigned run_seed) {
    ...
    if (waveType == WaveType::GERSTNER) {
        ...
        warmup_trackers([&](double t){ return sample_gerstner(t, trocho); }, sim_t, tracker, bias, rng, gauss);
        ...
        warmup_trackers([&](double t){ return sample_jonswap(t, jonswap_model); }, sim_t, tracker, bias, rng, gauss);
        ...
        auto fenton_params = FentonWave<4>::infer_fenton_parameters_from_amplitude(wp.height, 200.0f,
                                        2.0f*M_PI*wp.freqHz, wp.phase);
        FentonWave<4> fenton_wave(fenton_params.height, fenton_params.depth,
                                  fenton_params.length, fenton_params.initial_x);

        warmup_trackers([&](double t){ return sample_fenton(t, fenton_wave); }, sim_t, tracker, bias, rng, gauss);

        WaveSurfaceTracker<4> fenton_tracker(fenton_params.height, fenton_params.depth,
                                             fenton_params.length, fenton_params.initial_x, 5.0f, 0.1f);
        auto callback = [&](float time, float dt, float elevation,
                            float vertical_velocity, float vertical_acceleration, float x, float vx) {
