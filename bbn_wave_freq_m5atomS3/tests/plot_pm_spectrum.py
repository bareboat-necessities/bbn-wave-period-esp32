...
# === Find all exported spectrum CSVs for PM ===
files = glob.glob("*_pms_spectrum.csv")

if not files:
    print("No *_pms_spectrum.csv files found in this directory.")
    exit()
...
for fname in files:
    ...
    ax1.set_title(f"PM Stokes Directional Spectrum\n{os.path.basename(fname)}")
    ...
    ax2.set_title(f"PM Stokes Directional Spectrum (3D)\n{os.path.basename(fname)}")
