...
public:
    ...
    static constexpr Real kappa = 3 - N_STATE;
    ...
    void update(Real y_meas, Real dt) {
        ...
        x(2 * M) = std::max(x(2 * M), Real(2 * M_PI * 0.01));  
    ...
    Real estimatedHeave() const {
        Real heave = Real(0);
        Real omega = x(2 * M);
        omega = std::clamp(omega, Real(2 * M_PI * 0.01), Real(2 * M_PI * 100.0));
        for (int k = 1; k <= M; ++k) {
            int i = 2 * (k - 1);
            Real denom = k * omega;
...
private:
    ...
    SigmaMat predictSigmaPoints(const SigmaMat& sigma_points, Real dt) {
        ...
        for (int i = 0; i < SIG_CNT; ++i) {
            ...
            Vec x_pred = Vec::Zero();
            //Real clamped = std::clamp(x_sigma(2 * M), Real(2 * M_PI * 0.01), Real(2 * M_PI * 100.0));
    ...
    Real measurementModel(const Vec& x_sigma) {
        //Real omega = std::clamp(x_sigma(2 * M), Real(2 * M_PI * 0.01), Real(2 * M_PI * 100.0));
        Real omega = x_sigma(2 * M);
        Real y = 0;
        for (int k = 1; k <= M; ++k) {
            int idx = 2 * (k - 1);
            ...
            // Safe kw
            Real kw = k * omega;
            if (!std::isfinite(kw) || std::abs(kw) < Real(1e-6)) {            
                kw = Real(1e-6);
            }
            y += -kw * kw * cos_term;
        }
