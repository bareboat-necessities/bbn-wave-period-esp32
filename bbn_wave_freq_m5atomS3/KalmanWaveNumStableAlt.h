...
private:
    ...
    void adaptProcessNoise(float k_hat, float alpha_f = 0.001f) {
        ...
        float q1_val = q1_val_unclamped; //std::clamp(q1_val_unclamped, q2_at_f_high, q1_at_f_low);

        // q2 scales as 1/f (higher at low freq)
        float q2_val_unclamped = q2_at_f_low * (FREQ_L) / (f_safe);
        float q2_val = q2_val_unclamped; //std::clamp(q2_val_unclamped, q2_at_f_high, q2_at_f_low);
