    ...
    if ((a_no_spikes * a_no_spikes) < ACCEL_MAX_G_SQUARE_NO_GRAVITY) {
      ...
      if (t > warmup_time_sec(useMahony)) {
        ...
        if (useFrequencyTracker == Aranovskiy) {
          ...
          float f_byZeroCross = freqDetector.update(a_noisy, 1.0f, 0.1f, delta_t_inner); 
      ...
      if (isnan(freq) || isnan(freq_adj)) {
        ...
        SampleType sample = { .value = (false /*usedFreq*/ ? waveAltState.heave : heave), .timeMicroSec = now };
